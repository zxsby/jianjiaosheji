<template>
  <div>
    ...
    <Scroll @touchEnd="touchEnd" @scroll="scroll" :touch="true" :interactive="true" class="main-content" :probeType="2"
            :listenScroll="true" :data="RecommendMsg" ref="scroll">
      <div v-if="firstLunBo.length">
        <Silder>
          <div v-for="item in firstLunBo">
      ...
      _getRecommend () {
        this.$http.get('./api/recommend').then((resoult) => {
          if (resoult.data.resCode === ERR_OK) {
            ...
            //  console.log(this.RecommendMsg.length)
          }
        }).catch((err) => {
          console.log(err)
        })
